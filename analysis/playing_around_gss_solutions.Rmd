...
Looks like people without children are a bit happier. Let's now break it down by the respondent's gender:
```{r}
gss %>% drop_na(happy,childs) %>% 
  mutate(happiness = as.numeric(happy_rev)) %>%
  mutate(has_children = childs!=0) %>%
  group_by(sex,has_children) %>% summarize(hapiness=mean(happiness))
```

Men with kids are happier than men without kids. For women it's in the same direction, but by a smaller margin. Let's look at it by age instead, breaking it down into 40 or younger, between 41-59, and 61+. 
...
```
Looks like the benefit (to the extent it's real), cancels out for older age. This is quite surprising(!) (Likely many confounds here... it's not the effect of having kids per se, but other differences b/w people w/ and w/o kids, especially for the older groups)
...
```
Older women w/o kids are quite a bit happier than those with kids? Weird. But have a look at the sample sizes! How big a sample size is this cell?
...
gss %>% drop_na(realrinc,happy) %>% 
  mutate(realrinc = as.numeric(as.character(realrinc))*2.29, happiness = as.numeric(happy_rev)) %>%
...
gss %>% drop_na(realrinc,happy,degree) %>% 
  mutate(realrinc = as.numeric(as.character(realrinc))*2.29, 
...
```
Interesting... we see a u-shaped curve here... 
...
gss %>% drop_na(god,helpful) %>% 
  ...
  summarize(Others_are_selfish = round(mean(helpful=="LOOKOUT FOR SELF"),2),n=n())
```
Looks like more religious people think others areslightly more out for themseleves (can you think of possible confounds here?)
...
gss %>% drop_na(helpful,age_recoded) %>%
  ...
  group_by(decade) %>%
  summarize(mean_age=mean(age_recoded),Others_are_selfish = round(mean(helpful=="LOOKOUT FOR SELF"),2),n=n())
```
Wow, talk about big effects! The older people get, the more likely they are to think that people are helpful.  (Is this just a cohort effect, or is it really about age? You can check the full GSS dataset to find out!).
...
Once again, the coding is reversed making it counterintuitive to iterpret. So let's flip it back around

```{r}
gss$hapmar_rev <- fct_rev(gss$hapmar)
...
gss %>% drop_na(realrinc,realinc,hapmar) %>% 
  mutate(
    indiv_income = as.numeric(as.character(realrinc))*2.29,
    familiy_income = as.numeric(as.character(realinc))*2.29,
...
Surprisingly, only 20 people for whom data are available indicated that they're not too happy in their marriage! In this small group, it looks like the respondent is earning a slightly larger share of the family's income. Let's now break it down by men/women respondent's and those earning more vs. less than 50% of family income and look at the average reported marital happiness. 

Let's also include the mean respondent and family income levels as columns.
```{r}
gss %>% drop_na(realrinc,realinc,hapmar_rev) %>% 
  mutate(
    indiv_income = as.numeric(as.character(realrinc))*2.29,
    family_income = as.numeric(as.character(realinc))*2.29,
  ...
  summarize(mean_prop = mean(rel_income),
            ...
            n=n())
```
Couple observations: Men who earn >50% of the family income earn quite a bit more as individuals than women who earn >50% of the family income. As households, the highest earning group is one in which men earn <=50% though. Looking at marital happiness, the happiest group seems to be the one where men are earning >50% of the income and women who are earning <=50%. fwiw, these are all small non-reliable trends. Although individual happiness is associated with income, marital happiness, it seems, is not. 
...
```
Across all education levels, people endorse elephant evolution more than human evolution. The diference is somewhat smaller for the more educated respondents though.
...
gss %>% drop_na(polviews) %>% group_by(polviews) %>%
  summarize(evolved_humans = mean(as.numeric(evolved=="True"),na.rm=TRUE), 
               ...
               diff_humans_elephants = evolved_humans - evolved_elephants,n=n())
```
Pretty striking...  Endorsement of human evolution plummets as we go from liberal to conservative while endorsement of elephant evolution doesn't change much 
